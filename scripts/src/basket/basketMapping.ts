...
export function basketMapping(basket: Basket) {
    let totalQuantity = 0;
    let totalSum = 0;
    ...
    basket.getItems().forEach((el)=> {
        ...
        itemEl.querySelector('.item__delete-button').addEventListener('click', (ev) => removeItemListener(ev as HTMLEvent, basket));
    });
