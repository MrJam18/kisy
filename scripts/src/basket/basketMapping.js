...
import { removeItemListener } from "./removeItemListener.js";
export function basketMapping(basket) {
    let totalQuantity = 0;
    let totalSum = 0;
    const htmlList = document.querySelector('#items-list');
    htmlList.innerHTML = '';
    let discountSum = 0;
    basket.getItems().forEach((el) => {
        ...
        itemEl.querySelector('.item__add-button').addEventListener('click', (ev) => addQuantity(ev, basket));
        itemEl.querySelector('.item__subtract-button').addEventListener('click', (ev) => subtractQuantity(ev, basket));
        itemEl.querySelector('.item__delete-button').addEventListener('click', (ev) => removeItemListener(ev, basket));
    });
...
//# sourceMappingURL=basketMapping.js.map