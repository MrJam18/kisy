...
export function basketRender(basket) {
    if (!basket.isEmpty()) {
        let totalQuantity = 0;
        let totalSum = 0;
        ...
        basket.getItems().forEach((el) => {
            ...
            itemEl.querySelector('.item__delete-button').addEventListener('click', (ev) => removeItem(ev, basket));
        });
...
//# sourceMappingURL=basketRender.js.map